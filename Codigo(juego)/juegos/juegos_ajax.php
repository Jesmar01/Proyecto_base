<?php
	...
	if($action == 'ajax'){
		...
		$count_query   = mysqli_query($enlace,"SELECT count(*) AS numrows FROM juegos");
		if ($row= mysqli_fetch_array($count_query)){$numrows = $row['numrows'];}
		$total_pages = ceil($numrows/$per_page);
		$reload = 'registro_juego.php';
		//consulta principal para recuperar los datos
		$query = mysqli_query($enlace,"SELECT * FROM juegos order by nombre  LIMIT $offset,$per_page");
		...
		if ($numrows>0){
            ...
            <thead class="thead">
                <tr style="background-color: #f5f8fa;">
                        ...
                        <th style=" text-align:left;">Relacion proceso</th>
			...
			while($row = mysqli_fetch_array($query)){
				?>
					 <tr>
                  			 	...
                  			 	<td align="left"><b><?php echo $row['id_juegos']; ?></b></td>
            ...
            <a  data-toggle="modal" data-target="#dataUpdate"
                data-id_juegos="<?php echo $row['id_juegos']?>"
                data-nombre="<?php echo $row['nombre']?>" 
                data-descripcion="<?php echo $row['descripcion']?>"
                data-id_p_cognitivo="<?php echo $row['id_p_cognitivo']?>" >
                          <span class="glyphicon glyphicon-edit" style="color:#00a59c"></span>
                          </a>&nbsp;&nbsp;


      <a href="#" data-toggle="modal" data-target="#dataDelete" data-id_estudiante="<?php echo $row['id_juegos']?>">
